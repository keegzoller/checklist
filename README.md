...
- `refinements.css` is shared by the homepage and interior pages. `showcase.js` adds a horizontal project gallery with touch scrolling, mouse dragging, arrow navigation, automatic advance (resumes after arrow interaction), and reduced-motion handling. Photo enlargement uses a native modal dialog.
